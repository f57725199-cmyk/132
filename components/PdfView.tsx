    ...
    <div className="bg-slate-50 min-h-screen pb-20 animate-in fade-in slide-in-from-right-8">
       ...
       {activePdf ? (
           <div className="h-[calc(100vh-80px)] w-full bg-slate-100 relative">
               ...
               {contentData?.watermarkText && (
                   <div className="absolute inset-0 z-10 pointer-events-none flex items-center justify-center overflow-hidden">
                       <div className="w-full h-full flex flex-col items-center justify-center gap-24 select-none opacity-40">
                            {/* Repeating Watermark Pattern with Black Background as requested */}
                            {Array.from({length: 8}).map((_, i) => (
                                <div key={i} className="-rotate-12 transform">
                                    <span className="text-3xl md:text-5xl font-black text-gray-400 bg-black px-6 py-2 shadow-xl whitespace-nowrap uppercase tracking-widest">
                                        {contentData.watermarkText}
                                    </span>
                                </div>
                            ))}
